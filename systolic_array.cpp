...
void PE(hls::stream<int> &A_in, hls::stream<int> &B_in, hls::stream<int> &A_out, hls::stream<int> &B_out, int *C_out, int K, bool init)//, hls::stream<int> &C_out, int K)
{
	...
	if(init)
		C_local = 0;
	else//maybe after hardware unroll, we could delete this. Here, soft-sim maybe only see one PE.....
		C_local = *C_out;
	for(int k=0; k<K; k++)
	{
		A_in >> A_tmp;
...
void Load_A_stream(hls::stream<int> &lstream, int *in_array, int K_MIN, int i, bool load_en)
{
	for(int k=0; k<K_MIN; k++)
	{
		int tmp = 0;
		if(load_en)
			tmp = in_array[i*K_MIN + k];
		lstream << tmp;
//			A_inter[i][0] << A_local[i*K_MIN + k];
	}
}

void Load_B_stream(hls::stream<int> &lstream, int *in_array, int K_MIN, int N_MIN, int j, bool load_en)
{
		for(int k=0; k<K_MIN; k++)
		{
			int tmp = 0;
			if(load_en)
				tmp = in_array[k*N_MIN + j];
			lstream << tmp;
...
#pragma HLS DATAFLOW
	...
	{
#pragma HLS UNROLL
		Load_A_stream(A_inter[i][0], A_local, K_MIN, i, i<M_MIN);
	}

	for(int j=0; j<TILE_N; j++)
	{
#pragma HLS UNROLL
		Load_B_stream(B_inter[0][j], B_local, K_MIN, N_MIN, j, j<N_MIN);
	}
